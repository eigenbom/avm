...
function M.slice(src, src_index, src_count, separator, format)
	...
	if format then
		for i=src_index,max_index do
			strings[#strings+1] = string.format(format, src[i])
		end
	else
		for i=src_index,max_index do
...
function M.matrix(src, src_index, num_cols, num_rows, format, options)
	...
	for row=1,num_rows do
		...
		for col=1,num_cols do
			...
			if format then
				row_str[#row_str+1] = string.format(format, src[index])
			else
				row_str[#row_str+1] = tostring(src[index])
			end
		end
		result[#result+1] = table.concat(row_str, ", ")
	end
...
function M.tabulated(table_format, num_rows, column_1, ...)
	...
	local column_padding_string = string.rep(" ", column_padding)
	...
	for row_index=-1,num_rows do
		if row_index == 0 then
			...
			local row = row_index > 0 and {string.format("%" .. max_row_label_width .. "d", row_index)} or {""}
			for column_index=0,num_cols do
				local column = column_index==0 and column_1 or select(column_index, ...)

				if row_index == -1 then
					-- print label
					row[#row+1] = string.rep(" ", max_row_label_width) .. (column.label or "-")
				elseif row_index == 0 then
					...
					if index > max_index then
						...
						if column.format then
							row[#row+1] = string.format(column.format, table.unpack(column.data, index, index+group_size-1))
						else
							local entry = {}
							...
							row[#row+1] = table.concat(entry, " ")
						end
					end
				end
			end
			local row_as_string = table.concat(row, column_padding_string)
			result[#result+1] = row_as_string
			max_line_length = math.max(max_line_length, #row_as_string)
		end
	end
	-- Adjust header separator width
	result[2] = string.rep("-", max_line_length)
	return table.concat(result, "\n")
end
